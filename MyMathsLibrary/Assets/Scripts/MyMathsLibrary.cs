...
public class MyMathsLibrary
{
    ...
    public static bool LineIntersectsAABB(MyAABBCollider box, MyVector3 startPoint, MyVector3 endPoint, out MyVector3 intersectionPoint)
    {
        ...
        if (!AxisIntersectsAABB(MyVector3.right, box, startPoint, endPoint, ref lowest, ref highest))
            return false;

        if (!AxisIntersectsAABB(MyVector3.up, box, startPoint, endPoint, ref lowest, ref highest))
            return false;

        if (!AxisIntersectsAABB(MyVector3.forward, box, startPoint, endPoint, ref lowest, ref highest))
            return false;
